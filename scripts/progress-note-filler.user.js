...
After the admission, the patient was in stable condition, the physical examination showed no abnormal findings, the lab data revealed normal hemogram, coagulation, liver, and kidney function, electrolytes level are within normal limits. The CXR showed no definite lesion, sharp CP angle and no cardiomegaly. The EKG revealed normal sinus rhythm.`;
    ...
    function createUI() {
        ...
        style.textContent = `
            ...
            /* Lab 區塊的呈現切換（趨勢/表格），沿用 lab-summary 的 view 偏好。
               做成一體的 segmented control：矮而寬，並用外框+較亮的字提高在深綠底上的辨識度 */
            .ntuh-blk-seg {
                display: inline-flex;
            ...
            .ntuh-blk-seg button {
                ...
                font-family: inherit;
                white-space: nowrap;
                /* HIS 頁面 CSS 會強制 button 的 width/height（複製鈕也是靠 !important 壓過去的），
                   不擋的話這幾顆會變成又高又窄的方塊 */
                ...
                if (k === 'lab' && r && r.ok && r.variants) {
                    ...
                    var renderLabVariant = () => {
                        ...
                        viewSeg.querySelectorAll('button').forEach((b) => {
                            b.classList.toggle('on', b.dataset.val === getLabView());
                        });
                    };
                }
    ...
    // 疏密固定用緊湊：這個面板的欄寬本來就窄，寬鬆版會被撐開。
    // 也刻意**不寫回** spacing 偏好——那把 key 是跟檢驗報告頁共用的，
    // 這裡的顯示需求不該去改那邊的設定。
