...
以下腳本需要使用此格式的筆記：`discharge-note-filler`、`progress-note-filler`

（`admission-note-filler` 改用 [Admission Note 模板與規則](./admission-note-template.md)）

每個區塊以 `-----[區塊名]-----` 分隔，區塊順序不限，但名稱必須完全一致。
