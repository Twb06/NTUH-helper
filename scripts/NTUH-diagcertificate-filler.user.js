...
(function () {
    ...
    function handleConsentMessage(msg) {
        ...
        if (msg.error) {
            ...
            (msg.items || []).forEach(it => applySuggestedOperationNameByDate(it.opDate, it.operationName, it.sourceTitle));
    ...
    // 背景掃描手術同意書 PDF → 帶入建議手術名稱與診斷病名（按鈕與自動偵測共用）
    ...
    async function autoDetectRecords() {
        try {
            ...
            // 本次住院期間有手術 → 自動觸發同意書 PDF 讀取（入帳名稱背景掃描已退場）
            if (autoOps.length > 0) {
                triggerConsentScan();
            }
        } catch (e) {
